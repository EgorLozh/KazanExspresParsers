...
def getPage(page):
    ...
    response = requests.post('https://graphql.kazanexpress.ru/', headers=headers, json=json_data)
...
def get_page_mouses(page):
    ...
    for i, item in enumerate(response["data"]["makeSearch"]["items"]):
        ...
        available_amount = 0
        try:
            available_amount = getProduct(id)["data"]["product"]["skuList"][0]["availableAmount"]
        except:
            pass
...
count_of_products = getPage(1)["data"]["makeSearch"]["total"]
...
with open("mouses.json","w",encoding='utf-8') as file:
    ...
    for page in range(1, count_of_pages):
        data.extend(get_page_mouses(page))
    json.dump(data,file)
